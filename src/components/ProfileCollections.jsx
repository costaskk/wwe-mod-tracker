  ...
  return (
    <section className="panel soft-panel" id={sectionId}>
      <div className="panel-header with-actions">
        ...
        <button className="primary-button small-btn" onClick={onCreate} disabled={!session} type="button">
          Create collection
        </button>
      </div>
          ...
          {collections.map((collection) => (
            <article className="collection-card" key={collection.id}>
              {collection.cover_url ? (
                <img className="collection-cover" src={collection.cover_url} alt={collection.name} />
              ) : (
              ...
              <div className="collection-body">
                ...
                <div className="muted-text small-text">
                  {collection.items?.length || 0} items · Updated {formatDate(collection.updated_at)}
                </div>
                ...
                <div className="collection-actions wrap-actions">
                  <button className="secondary-button small-btn" onClick={() => onOpen(collection)} type="button">
                    Browse
                  </button>

                  {collection.visibility === 'public' ? (
                    <button className="ghost-button small-btn" onClick={() => onShare(collection)} type="button">
                      Share
                    </button>
                  ) : null}

                  <button className="ghost-button small-btn" onClick={() => onEdit(collection)} type="button">
                    Edit
                  </button>

                  <button className="ghost-button small-btn" onClick={() => onDelete(collection)} type="button">
                    Delete
                  </button>
                </div>
              </div>
